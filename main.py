...
from utils.FileLookupService import FileLookupService
from utils.detection_tools import list_apps, get_bundle_identifier
...
def main():
    ...
    if 0 <= choice < len(apps):
        ...
        if bundle_identifier:
            app_model = AppModel(apps[choice].strip(".app"), bundle_identifier, app_path)
            lookup_service = FileLookupService(app_model)

            app_model.related_files = lookup_service.find_app_related_files()
